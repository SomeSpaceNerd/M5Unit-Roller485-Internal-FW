...
This firmware has been modified to support 1000000 BPS (1Mbps) RS-485 communication for faster control
...
1. Download the 'ROLLER485.bin' file from the [releases page](https://github.com/SomeSpaceNerd/M5Unit-Roller485-Internal-FW/releases)
2. Using a tool like [M5 DAPLink](https://docs.m5stack.com/en/guide/develop_tools/daplink) or an ST-LINK + STM32CubeProgrammer, erase the whole chip and flash the ROLLER485.bin file to address 0x08000000
3. Disconnect the motor from the programer and restart it to ensure it's running the new firmware correctly
4. Because the flash was erased the motor must be recalibrated and reconfigured
    1. Configure the motor to I2C mode (it is in I2C mode by default immediately after programming) and [recalibrate the encoder](https://docs.m5stack.com/en/guide/motor_ctl/roller485/calibration)
    2. Follow [these steps](https://docs.m5stack.com/en/guide/motor_ctl/roller485/roller485#2device-configuration) to reconfigure the motor's communication interface, speed, pid, etc
